...
use embassy_stm32::spi::{Spi, Config as SpiConfig, BitOrder, MODE_0};
...
use super::{DISPLAY_WIDTH, DISPLAY_HEIGHT, FRAMEBUFFER_SIZE};
    ...
    BUSY: embassy_stm32::gpio::Pin,
{
    ...
    async fn load_lut_partial(&mut self) -> Result<(), DisplayError> {
        ...
        const LUT_PARTIAL: [u8; 159] = [
            ...
            0x00, 0x00, 0x00, 0x22, 0x17, 0x41,
            0xB0, 0x32, 0x36, 0x00, 0x00, 0x00,
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    ...
    async fn set_ram_window(&mut self, x_start: u16, y_start: u16, x_end: u16, y_end: u16) -> Result<(), DisplayError> {
    ...
    BUSY: embassy_stm32::gpio::Pin,
{
    async fn init(&mut self) -> Result<(), Self::DriverError> {
        ...
        self.set_ram_window(0, 0, DISPLAY_WIDTH as u16 - 1, DISPLAY_HEIGHT as u16 - 1).await?;
    ...
    BUSY: embassy_stm32::gpio::Pin,
{
    ...
    async fn refresh_partial(&mut self) -> Result<(), Self::DriverError> {
        ...
        if self.partial_refresh_count >= 5 {
            defmt::info!("Triggering full refresh to clear ghosting after {} partial refreshes", self.partial_refresh_count);
