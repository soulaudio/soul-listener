...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("Display size after rotation: {}×{}", size.width, size.height);
    ...
    Text::new("TEST - Portrait Mode", Point::new(20, 35), header_style)
        .draw(&mut emulator)?;

    // Draw some test content
    let text_style = MonoTextStyle::new(&FONT_10X20, Gray4::BLACK);
    Text::new(&format!("Display: {}x{}", size.width, size.height), Point::new(20, 100), text_style)
        .draw(&mut emulator)?;
    Text::new("This should be PORTRAIT (tall)", Point::new(20, 130), text_style)
        .draw(&mut emulator)?;
    Text::new("Width should be LESS than height", Point::new(20, 160), text_style)
        .draw(&mut emulator)?;
